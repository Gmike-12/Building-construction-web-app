...
if (isset($_POST['submit'])) {
    ...
    $stmt = $con->prepare("SELECT * FROM const_sites WHERE accessToken = ?");
    $stmt->bind_param("s", $enteredToken);
    $stmt->execute();
    $result = $stmt->get_result();

    if ($result->num_rows > 0) {
        // Token exists, proceed to delete the row
        $deleteStmt = $con->prepare("DELETE FROM const_sites WHERE accessToken = ?");
        $deleteStmt->bind_param("s", $enteredToken);
        ...
        error_reporting(0);
        echo "<script>window.location.href = 'http://localhost/Foremanapp/Delete.php'</script>";
    }
...
<body>
    ...
    <div class="form-group">
                <label for="token">Enter Your Access Token To Delete Your Posted Job:</label>
